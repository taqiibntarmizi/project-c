...
vector<double> mean_diff_sampling_distribution(vector<double> data_A, vector<double> data_B) {
  ...
  for (int i = 0; i < 1000; ++i) {
    ...
    double diff = mean(resample_A) - mean(resample_B);
    mean_diff.push_back(diff);
  }
  return mean_diff;
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  ...
  double low_bound = ((1-width) / 2 )  ;
  double upper_bound = ((1+width) / 2 )  ;

  double lower_percentile = percentile (v,low_bound);
  double upper_percentile = percentile (v, upper_bound);
